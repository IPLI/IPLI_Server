...
void run_detector(int argc, char **argv, objcnt *objects, FILE *file_pointer, network r_net, char * weights)
{
    printf("penguin");
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box);
        ...
        //==============================
        /*r_net = parse_network_cfg_custom(cfg, 1, 1);    // set batch=1
        if (weights) {
            load_weights(&r_net, weights);
        }
        fuse_conv_batchnorm(r_net);
        calculate_binary_weights(r_net);
        srand(2222222);*/
        //===============================
        demo(cfg, weights, thresh, hier_thresh, cam_index, filename, names, classes, frame_skip, prefix, out_filename,
            mjpeg_port, json_port, dont_show, ext_output, letter_box, objects, file_pointer, r_net);
        free_list_contents_kvp(options);
        free_list(options);
    }
    else printf(" There isn't such command: %s", argv[2]);
    printf("run detector end...........d\n");
}
