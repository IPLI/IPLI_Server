...
// ----------------------------------------
/*
cap_cv* get_capture_stream_data(char** stream)
{
    cv::VideoCapture* cap = NULL;
    try {
        char ** data = [BUFSIZE][16];
        //int length = recvall(clntSock,message, 16);
        //strcpy(data[cnt++], message);
        recvall(clntSock, data, 16-1,0);
        //stringDatas to uint 8 ..굳이 필요한가?char그대로 써도 될것같은데...
        
        cap = new cv::VideoCapture(data);
...
*/
/*
char** recvall(SOCKET clntSock, char **buf, int len, int flags){
    int recvLen;
    while ((recvLen = recv(clntSock, buf[cnt++], BUFSIZE-1,0))!=0){
        // recieve Error 처리
...
// Draw Detection
// ====================================================================
void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, FILE *file_pointer, int *buffer, objcnt *objects)
{
    try {
        //insert code start
        
        //FILE *file_pointer;
        //file_pointer = fopen("C:\\Users\\JunSeop\\Desktop\\stat\\a.txt", "a+");
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                //만약 names[class_id] == 학습시킨 객체 라면 해당 객체를 파일에 넣는다.
                if (!strcmp(names[class_id], "coca-cola"))
                    buffer[0]++;
                else if (!strcmp(names[class_id], "butter_ring"))
                    buffer[1]++;
                else if (!strcmp(names[class_id], "ramen"))
                    buffer[2]++;
                else if (!strcmp(names[class_id], "downy"))
                    buffer[3]++;
                else if (!strcmp(names[class_id], "corn-frost"))
                    buffer[4]++;

                for (int z = 0; z < 5; z++)
                {
                    if ((buffer[z] > 10) && (objects[z].cnt == 0))
                    {
                        objects[z].cnt++;
                        //buffer[z] = 0;
                        /*
                        fputs(names[class_id], file_pointer);
                        fputs(" is here!\n", file_pointer);
                        printf("[buffer is %d]", buffer[z]);
                        buffer[z] = 0;
                        */
                    }
                }
