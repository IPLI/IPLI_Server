...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output, int letter_box_in, objcnt *objects, FILE *file_pointer, network r_net)
{
    ...
    /*else if () {
        ...
        cap = get_capture_stream_data(stream);
    }*/
    else {
    ...
    layer l = r_net.layers[r_net.n-1];
    int j;
    ...
    int z = 0;
    int buffer[5] = { 0 };//버퍼 버퍼가 일정 수에 도달하면 그 객체가 있는것으로 판단함
    //int how_many_obj[3] = { 0 };//인덱스가 어떤 객체인지를, 인덱스의 요소가 객체의 개수를 저장 ex) 콜라가 1번인덱스고 2개있으면 how_many_obj[1] = 2이다.
    ...
    while(z < 40){
        ...
        ++count;
        {
            ...
            draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, file_pointer, buffer, objects);
    ...
    free_ptrs((void **)names, r_net.layers[r_net.n - 1].classes);
...
#else
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output, int letter_box_in, objcnt *objects, FILE *file_pointer)
{
